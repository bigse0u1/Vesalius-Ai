...
class _RawKeyboard:
    """Read keystrokes from /dev/tty without X11, works over SSH."""
    def __init__(self):
        self._pressed_time = {}
        self._running = False
        self._thread = None
            ...
            if r:
                try:
                    ...
                    if ch == '\x03':
                        ...
                        self._pressed_time[ch] = time.perf_counter()
                except Exception:
                    pass

    def get_pressed(self):
        now = time.perf_counter()
        return {k for k, t in self._pressed_time.items() if now - t < 0.15}
...
class XLerobot(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        try:
            self._raw_kb.start()
            logger.info("Keyboard listener started for base wheel control (raw tty).")
        except Exception as e:
    ...
    def configure(self):
        ...
        for name in self.left_arm_motors:
            ...
            # Operating_Mode write resets Goal_Velocity to 0; restore to allow movement
            self.bus1.write("Goal_Velocity", name, 3000)
        ...
        for name in self.right_arm_motors:
            ...
            # Operating_Mode write resets Goal_Velocity to 0; restore to allow movement
            self.bus2.write("Goal_Velocity", name, 3000)
        
        for name in self.base_motors:
            self.bus2.write("Operating_Mode", name, OperatingMode.VELOCITY.value)
        
        
        self.bus1.enable_torque(num_retry=5)
        self.bus2.enable_torque(num_retry=5)
        # Disable torque on base motors - not needed for arm teleop
        for name in self.base_motors:
            try:
                self.bus2.disable_torque([name])
            except Exception:
                pass
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        def _safe_read(bus, data_name, motors, num_retry=5, cache_attr=None):
            try:
                result = bus.sync_read(data_name, motors, num_retry=num_retry)
                if cache_attr:
                    setattr(self, cache_attr, result)
                return result
            except Exception:
                if cache_attr and hasattr(self, cache_attr):
                    return getattr(self, cache_attr)
                return {m: 2048 for m in motors}
        left_arm_pos = _safe_read(self.bus1, "Present_Position", self.left_arm_motors, num_retry=0, cache_attr="_cache_left_arm")
        # Individual reads for bus2 to avoid GroupSyncRead failures
        right_arm_pos = {}
        for _m in self.right_arm_motors:
            try:
                right_arm_pos[_m] = self.bus2.read("Present_Position", _m)
            except Exception:
                right_arm_pos[_m] = self._cache_right_arm.get(_m, 2048) if hasattr(self, "_cache_right_arm") else 2048
        self._cache_right_arm = right_arm_pos
        head_pos = _safe_read(self.bus1, "Present_Position", self.head_motors, num_retry=0, cache_attr="_cache_head")
        base_wheel_vel = {m: 0 for m in self.base_motors}
    ...
    def get_camera_observation(self):
        ...
        for cam_key, cam in self.cameras.items():
            start = time.perf_counter()
            try:
                obs_dict[cam_key] = cam.async_read(timeout_ms=2000)
            except TimeoutError:
                logger.warning(f"Camera {cam_key} timed out, skipping frame")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        # Remap bi_so_leader keys: left_X.pos -> left_arm_X.pos, right_X.pos -> right_arm_X.pos
        remapped = {}
        for key, value in action.items():
            if key.startswith("left_") and not key.startswith("left_arm_"):
                remapped["left_arm_" + key[len("left_"):]] = value
            elif key.startswith("right_") and not key.startswith("right_arm_"):
                remapped["right_arm_" + key[len("right_"):]] = value
            else:
                remapped[key] = value
        action = remapped
        ...
        if not base_goal_vel:
            active_keys = self._raw_kb.get_pressed()
            base_goal_vel = self._from_keyboard_to_base_action(active_keys)
        ...
        base_wheel_goal_vel = {k: v for k, v in base_wheel_goal_vel.items() if k in self.bus2.motors}
    ...
    def disconnect(self):
        ...
        try:
            ...
            logger.warning(f'stop_base failed on disconnect: {e}')
